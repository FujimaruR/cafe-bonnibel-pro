            ...
            <div className="md:col-span-2 space-y-4">
              {items.map(p => (
                <div key={p.name} className="bg-crema p-4 rounded-2xl border border-beige/70 flex items-center gap-4">
                  ...
                  <button onClick={() => remove(p.name)} className="text-crema underline ml-2">Quitar</button>
                </div>
              ))}
              <button onClick={clear} className="text-sm text-crema underline">Vaciar carrito</button>
            </div>
